...
const MAX_DIM = 1600;
// Render free = 0.1 CPU + 512MB RAM, so the WORK is budgeted by SOURCE pixels:
// keptFrames x sourceW x sourceH. Bigger GIFs get temporally SUBSAMPLED (every
// n-th frame) so the full loop is preserved at a lower framerate instead of
// truncating the animation. 6M px decodes in seconds even on 0.1 CPU.
const FRAME_BUDGET = 6e6;
const MAX_KEPT = 80;
// Built profiles are cached on disk: the heavy work is paid ONCE per gif,
// surviving restarts of the same instance.
const DISK_DIR = path.join(__dirname, '..', 'data', 'gifcache');
const DISK_MAX_FILES = 12;
...
function diskPath(url, w, h) {
  const hsh = crypto.createHash('sha1').update(url).digest('hex').slice(0, 20);
  return path.join(DISK_DIR, `${hsh}@${w}x${h}.json`);
}

function loadDisk(url, w, h) {
  try {
    const j = JSON.parse(fs.readFileSync(diskPath(url, w, h), 'utf8'));
    if (j && j.status === 'ready' && Array.isArray(j.frames) && j.frames.length && Array.isArray(j.timeline)) return j;
  } catch {}
  return null;
}

function saveDisk(url, w, h, entry) {
  try {
    fs.mkdirSync(DISK_DIR, { recursive: true });
    fs.writeFileSync(
      diskPath(url, w, h),
      JSON.stringify({ status: 'ready', w, h, url, frames: entry.frames, timeline: entry.timeline, total: entry.total, palette: entry.palette, box: entry.box })
    );
    // keep the directory bounded (drop oldest by mtime)
    const files = fs.readdirSync(DISK_DIR).map((f) => {
      const p = path.join(DISK_DIR, f);
      return { p, t: fs.statSync(p).mtimeMs };
    });
    if (files.length > DISK_MAX_FILES) {
      files.sort((a, b) => a.t - b.t);
      for (const f of files.slice(0, files.length - DISK_MAX_FILES)) {
        try { fs.unlinkSync(f.p); } catch {}
      }
    }
  } catch (e) {
    console.error('[cc-gif] disk cache save failed:', e.message);
  }
}
...
async function decodeGif(url) {
  ...
  const maxFrames = Math.max(8, Math.min(MAX_KEPT, Math.floor(FRAME_BUDGET / (W * H))));
  const stride = Math.max(1, Math.ceil(imageFrames.length / maxFrames));
  console.log(`[cc-gif] ${W}x${H} gif, ${imageFrames.length} frames -> keeping ${Math.min(imageFrames.length, maxFrames)} (stride ${stride})`);
...
async function build(url, w, h, deadline) {
  const chk = (phase) => {
    if (deadline && Date.now() > deadline) {
      throw new Error('this gif is too heavy for the free server - try a smaller one');
    }
    if (phase) console.log(`[cc-gif] phase: ${phase}`);
  };
  chk('start');
  const t0 = Date.now();
  ...
  chk('dithered');
  console.log(`[cc-gif] built ${w}x${h} profile: ${built.length} frames in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
...
function ensure(url, kind, w, h) {
  ...
  if (!entry) {
    ...
    cache.set(key, entry);
    const cached = loadDisk(url, w, h);
    if (cached) {
      console.log(`[cc-gif] ${w}x${h} profile for ${url.slice(0, 60)}: disk cache hit`);
      Object.assign(entry, cached, { status: 'ready' });
      return entry;
    }
    console.log(`[cc-gif] building ${w}x${h} profile for ${url.slice(0, 90)}`);
    const deadline = Date.now() + 150000; // watchdog: never spin forever
    chain = chain
      .then(() => build(url, w, h, deadline))
      .then((res) => {
        Object.assign(entry, res, { status: 'ready' });
        saveDisk(url, w, h, res);
      })
      .catch((e) => {
        entry.status = 'error';
...
// Accept a profile built by a BROWSER (fast CPU!) and cache it as if we made it.
// Validated hard: rows must RLE-sum to w, sane frame count, palette 48 bytes.
function injectBuilt(url, w, h, entry) {
  if (!url || !Number.isInteger(w) || !Number.isInteger(h)) throw new Error('bad profile id');
  if (!entry || entry.status !== 'ready' || !Array.isArray(entry.frames) || !entry.frames.length) {
    throw new Error('profile missing frames');
  }
  if (entry.frames.length > 200) throw new Error('too many frames');
  if (!Array.isArray(entry.palette) || entry.palette.length !== 48 || !entry.palette.every((v) => Number.isInteger(v) && v >= 0 && v <= 255)) {
    throw new Error('bad palette');
  }
  if (!Array.isArray(entry.timeline) || !(entry.total > 0)) throw new Error('bad timeline');
  for (const f of entry.frames) {
    if (!Array.isArray(f.lines) || f.lines.length !== h) throw new Error('bad row count');
    if (!(f.delay >= 10 && f.delay <= 5000)) throw new Error('bad delay');
    for (const line of f.lines) {
      let sum = 0;
      for (const part of String(line).split(',')) {
        const n = parseInt(part.slice(1), 10);
        if (!/^[0-9a-f][0-9]+$/.test(part) || !(n > 0)) throw new Error('bad RLE row');
        sum += n;
      }
      if (sum !== w) throw new Error('RLE row does not sum to monitor width');
    }
  }
  const key = `${url}@${w}x${h}`;
  const existing = cache.get(key);
  const fresh = existing || { status: 'building', w, h, url };
  Object.assign(fresh, entry, { status: 'ready' });
  cache.set(key, fresh);
  saveDisk(url, w, h, entry);
  console.log(`[cc-gif] browser-built profile accepted for ${w}x${h} (${entry.frames.length} frames)`);
  return fresh;
}

function buildingSince(url, w, h) {
  const e = cache.get(`${url}@${w}x${h}`);
  return e && e.status === 'building' ? e : null;
}

module.exports = { ensure, frameIndexAt, injectBuilt, buildingSince };
