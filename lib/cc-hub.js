...
const ccSizes = new Map(); // "wxh" -> last seen at (all monitors ever connected)
...
function onMessage(ws, data) {
  ...
  if (m.t === 'hello') {
    ws.cc.w = Math.min(600, Math.max(2, parseInt(m.w, 10) || 7));
    ws.cc.h = Math.min(300, Math.max(2, parseInt(m.h, 10) || 5));
    ccSizes.set(`${ws.cc.w}x${ws.cc.h}`, Date.now());
    ws.cc.audio = !!m.audio;
    ws.cc.cursor = -1;
    ws.cc.lastKey = null;
    send(ws, {
  ...
  try {
    ...
    for (const [k, socks] of profiles) {
      ...
      if (entry.status === 'video') {
        ...
        // push the custom palette before the first frame of each new gif
        const palKey = `${k}|${gif.url}`;
        for (const ws of socks) {
          if (entry.palette && ws.cc.palKey !== palKey) {
            ws.cc.palKey = palKey;
            send(ws, { t: 'pal', c: entry.palette });
          }
        }
        ...
        // ask any open browser tab to build this profile with its fast CPU
        if (!entry.needSent) {
          entry.needSent = true;
          entry.startedAt = entry.startedAt || Date.now();
          io.emit('cc:need', { url: gif.url, w, h });
        }
        // nothing took the job and we have waited too long -> say so
        if (entry.startedAt && Date.now() - entry.startedAt > 150000) {
          const hint = 'stuck! open the website once (any Chrome tab builds it), or try a smaller gif';
          for (const ws of socks) {
            if (ws.cc.lastKey !== 'stuck:' + gif.url) {
              ws.cc.lastKey = 'stuck:' + gif.url;
              send(ws, { t: 'gife', error: hint });
            }
          }
        }
      }
...
module.exports = {
  ...
  sizes: () => [...ccSizes.keys()],
  emitNeed: (url, w, h) => io && io.emit('cc:need', { url, w, h }),
};
